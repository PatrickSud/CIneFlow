...
- **Migração para TypeScript (em andamento, incremental):** já convertidos `src/lib/library.ts` e `src/lib/tmdb.ts` com tipos, e criado `src/types.ts` com a interface `Item` e as uniões `Tipo`/`Status`. `tsconfig.json` usa `allowJs`, então o `App.jsx` continua funcionando enquanto a migração avança. Verificação com `npm run typecheck` (passa em modo strict). Passo 2: extraídos `src/lib/contentTypes.ts` e os componentes tipados `StarRating.tsx` e `Toast.tsx`. Passo 3: extraído `ItemCard.tsx` + lógica `bestTmdbMatch` (com testes). Passo 4: extraídos `Dashboard.tsx` e `DetailModal.tsx` (tipados), e o cabeçalho passou a ter um menu único **Biblioteca** (adicionar/configurar/importar/backup). O `App.jsx` caiu de ~2.500 para ~2.130 linhas. Próximos passos: extrair o grande modal de formulário (Adicionar/Editar) e os modais menores, e por fim converter o `App` para `.tsx`.
