...
- **Migração para TypeScript (em andamento, incremental):** já convertidos `src/lib/library.ts` e `src/lib/tmdb.ts` com tipos, e criado `src/types.ts` com a interface `Item` e as uniões `Tipo`/`Status`. `tsconfig.json` usa `allowJs`, então o `App.jsx` continua funcionando enquanto a migração avança. Verificação com `npm run typecheck` (passa em modo strict). Passo 2: extraídos `src/lib/contentTypes.ts` e os componentes tipados `StarRating.tsx` e `Toast.tsx`. Passo 3: extraído `ItemCard.tsx` + lógica `bestTmdbMatch` (com testes). Passo 4: extraídos `Dashboard.tsx` e `DetailModal.tsx`; menu único **Biblioteca** no cabeçalho. Passo 5: removido o tema claro (app só escuro) e extraídos os modais menores tipados — `ConfirmDialog` (reutilizável para excluir/atualizar), `TagManagerModal`, `TmdbHelpModal`, `IosInstallHelp`. O `App.jsx` caiu para ~1.960 linhas (9 componentes tipados). Passo 6: **`App.jsx` convertido para `App.tsx`** e tipado em modo strict — `useState` com generics (`Item[]`, uniões `Tipo`/`Status`, etc.), parâmetros de handlers e refs tipados. O `tsc --noEmit` cobre agora o app inteiro e passa sem erros, então a passagem de props para os componentes é validada. Passo 7 (final): extraído o grande modal `AddEditModal.tsx` (Adicionar/Editar/Importar/Buscar). **Migração concluída** — o `App.tsx` (~1.600 linhas) passou a ser o orquestrador (estado + handlers) e toda a UI está em 10 componentes tipados, com `tsc --noEmit` strict cobrindo o app inteiro. O que vier agora é polimento opcional (mais testes, dividir o `App` em hooks/partes menores).
